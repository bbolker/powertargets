...
|----------|---------|---------|
| $s$ | 1 | SESOI ("smallest effect size of interest") |
| $\delta$ | 1  |unscaled effect size (difference between means) |
| $\sigma$ | 1 | SESOI ("smallest effect size of interest") |
...
plotfun(f_lengthen(sim1))
```

Alternatively:

```{r sim1-plot-stack, fig.width = 8, message = FALSE}
plotfun(f_lengthen(sim1), stack = TRUE)
```
...
sim2 <- lapply(nvec, tabfun, delta=1.5, nsim = 10000) |> do.call(what = rbind)
attr(sim2, "pars") <- c(delta = 1.5, s = 1)
plotfun(f_lengthen(sim2))
```

```{r sim2-plot-2, fig.width = 8}
plotfun(f_lengthen(sim2), stack = TRUE)
```
...
* other examples?
* it would be elegant and computationally efficient to figure out the analytical results for this (i.e., probably in terms of non-central $\chi^2$ distributions - @owenSpecial1965 or @juliousSample2004 seem like useful starting points), although 10,000 simulations take only about `r round(t0[["elapsed"]], 2)` seconds
